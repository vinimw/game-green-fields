...
export const aliveCountByType = (
  monsters: readonly { state: { alive: boolean; type: MonsterType } }[],
  type: MonsterType,
) => monsters.filter((m) => m.state.alive && m.state.type === type).length;
export const allowedSpawnCount = (
  totalAlive: number,
  typeAlive: number,
  groupSize: number,
  maxTotal: number,
  ...
  Math.max(0, Math.min(groupSize, maxTotal - totalAlive, maxType - typeAlive));
export const selectSpawnType = (
...
export class MonsterPopulationSystem {
  ...
  constructor(
    ...
    private createMonster: (type: MonsterType, position: Vec2) => Monster,
    private playerPosition: () => Vec2,
    pending: PendingRespawn[] = [],
    private random: () => number = Math.random,
  ) {
    this.pendingRespawns = pending.map((p) => ({ ...p }));
  }
  get totalAlive() {
    return this.monsters.filter((m) => m.state.alive).length;
  }
  ...
  initialize() {
    ...
    while (this.totalAlive < target && guard-- > 0) {
  ...
  onMonsterKilled(type: MonsterType) {
    ...
    this.pendingRespawns.push({
      type,
      remainingTimeMs:
        this.rules[type].respawnDelayMs ??
        MONSTER_SPAWN_CONFIG.respawn.defaultDelayMs,
    });
  }
  ...
  spawn(type: MonsterType) {
    ...
    const count = allowedSpawnCount(
      ...
      this.rules[type].spawnGroupSize,
      type === "bear"
        ? Number.MAX_SAFE_INTEGER
        : MONSTER_SPAWN_CONFIG.maxMonsters,
      this.rules[type].maxAlive,
    );
    if (count === 0) return 0;
    const player = this.playerPosition(),
      center = this.positions.findValidSpawnPosition(player);
    if (!center) return 0;
    let spawned = 0;
    for (let i = 0; i < count; i++) {
      const position =
        i === 0
          ? center
          : this.positions.findValidSpawnPosition(
              player,
              center,
              MONSTER_SPAWN_CONFIG.groupRadius,
            );
      if (!position) continue;
      this.monsters.push(this.createMonster(type, position));
      spawned++;
    }
    return spawned;
  }
  selectType() {
    return selectSpawnType(
      {
        crawler: this.aliveByType("crawler"),
