...
describe('Ghost monster',()=>{
  it('has the requested stats and solitary spawn limit',()=>{expect(MONSTERS_CONFIG.ghost).toMatchObject({level:3,health:60,damage:60,experienceReward:50,movementSpeed:2,detectionRadius:18,coinDrop:{chance:.4,amount:20}});expect(MONSTERS_CONFIG.ghost.detectionRadius).toBeGreaterThan(MONSTERS_CONFIG.wailer.detectionRadius);expect(MONSTER_SPAWN_CONFIG.monsters.ghost).toMatchObject({maxAlive:5,spawnGroupSize:1});});
