...
describe("Mage Skill Books",()=>{
  ...
  it("collects the Arrow Rain book",()=>{const engine=new NullEngine(),scene=new Scene(engine),player=createPlayerState("archer"),rolls=[.9,.9,.2,.4],drops=new SkillBookPickupSystem(scene,player,()=>({x:2,z:3}),()=>{},()=>rolls.shift()??.9);expect(drops.tryDrop("crawler",{x:2,z:3})).toBe(true);drops.update(.01);expect(player.inventory).toContain("arrow-rain-skill-book");scene.dispose();engine.dispose();});
  it("collects the Ricochet Arrow book",()=>{const engine=new NullEngine(),scene=new Scene(engine),player=createPlayerState("archer"),rolls=[.9,.9,.9,.2,.4],drops=new SkillBookPickupSystem(scene,player,()=>({x:2,z:3}),()=>{},()=>rolls.shift()??.9);expect(drops.tryDrop("crawler",{x:2,z:3})).toBe(true);drops.update(.01);expect(player.inventory).toContain("ricochet-arrow-skill-book");scene.dispose();engine.dispose();});
